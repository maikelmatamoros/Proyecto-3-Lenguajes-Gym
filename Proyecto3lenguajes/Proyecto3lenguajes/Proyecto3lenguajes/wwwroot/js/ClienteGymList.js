...
function cargarPagina(resultados) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        $("#contenedor").append(
            ...
            '<div class="card card-body h-30" style="background-color:rgba(10, 129, 16,0.4)!important;border-radius:20px;">' +

            '<img src="' + ruta + '" style="height:40vh"/>' +

            '<h4 class="card-title text-center" style="color: white;">' + array[i].centro_nombre + '</h4>' +

            '<h6 class="card-subtitle mb-2 text-center" style="color: white;">' + array[i].centro_ubicacion+'</h6 > ' +
